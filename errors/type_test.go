	...
	testCases := []struct {
	...
	}{
		{
			...
			expected: HapiError{
				...
				Message:   "Mindhunter is a pretty good show",
			},
		},
		{
			...
			expected: HapiError{
				...
				Message:   "42 is the answer to life",
			},
		},
	}
	...
	testCases := []struct {
		...
		},
		{
			...
			expected: HapiError{
				...
				Message:   "42 is still the answer to life",
			},
		},
	}
	for _, tc := range testCases {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expected: HapiError{
				...
				Message:   "gonna wrap dat og error",
			},
		},
		{
	...
	testCases := []struct {
		...
		},
		{
			...
			expected: HapiError{
				...
				Message:   "some awesome hapi error",
			},
		},
	}
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			actual := CastToHapiError(tc.errToCast)
